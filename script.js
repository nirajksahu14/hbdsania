...
envelope.addEventListener("click", () => {
  ...
  envelopeStage.classList.add("fullscreen-open");
  requestAnimationFrame(() => {
    envelope.classList.add("open");
    envelope.setAttribute("aria-expanded", "true");
  });
  burstAt(window.innerWidth/2, window.innerHeight*.48, 42, true);
  setTimeout(startTypewriter, 1650);
});

function startTypewriter() {
  if (typewriterStarted) return;
  typewriterStarted = true;
  letterScroll.textContent = "";
  typeCaret.style.display = "inline-block";
  const letterChars = Array.from(letterText);
  let index = 0;

  const typeNext = () => {
    if (index >= letterChars.length) {
    ...
    const char = letterChars[index];
    letterScroll.textContent += char;
    index += 1;
    ...
    let delay = 16;
    if (char === "\n") delay = 95;
    else if (/[.!?]/.test(char)) delay = 72;
    else if (/[,—]/.test(char)) delay = 40;
    else delay += Math.random() * 15;
...
finalRevealBtn.addEventListener("click", () => {
  ...
  cinematicTransition(() => {
    ...
    envelopeStage.classList.remove("fullscreen-open");
    showScene("final");
    launchFinalConfetti();
  });
});
